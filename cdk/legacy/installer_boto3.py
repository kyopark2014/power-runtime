...
# Configuration
project_name = "gsolution" # at least 3 characters
region = "us-west-2"
...
def _ecs_agent_runtime_resource_arns() -> List[str]:
    ...
    Root installer project_name (e.g. gsolution) can differ from the Agent Runtime
    installer projectName (e.g. strands-runtime → strands_runtime-*). Include both
    naming conventions plus any explicit agent_runtime_arn from local configs.
    """
    root = os.path.dirname(os.path.abspath(__file__))
    app_config = _read_local_json_config(os.path.join(root, "application", "config.json"))
    runtime_config = _read_local_json_config(
        os.path.join(root, "runtime_agent", "strands", "config.json")
    )
    ...
    for config in (app_config, runtime_config):
                ...
                "Statement": [
                    {
                        ...
                        # (e.g. gsolution vs strands_runtime-*). See _ecs_agent_runtime_resource_arns.
...
def main():
    ...
    try:
        ...
        # (e.g. strands_runtime-*), not only root project_name (gsolution).
        ecs_roles = create_ecs_roles()
