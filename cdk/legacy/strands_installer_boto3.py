...
def _resolve_project_name(config: dict) -> str:
    """Return canonical project name from root installer (not a stale runtime config).

    Runtime config used to keep a leftover projectName (e.g. strands-runtime) even
    after the repo was deployed as gsolution, which caused AgentCore Role/S3 ARNs
    to point at the wrong project.
    """
    # Prefer root installer.py project_name, then application/config.json.
...
def _ensure_config_defaults(config: dict) -> dict:
    ...
    if updated.get("projectName") != project_name:
        print(
            f"  ✓ Syncing projectName '{updated.get('projectName')}' → '{project_name}' "
            "(from root installer / application config)"
        )
    ...
    # Derive storage bucket from project when still missing.
    if not (updated.get("s3_bucket") or "").strip():
        account_id = updated.get("accountId") or ""
        region = updated.get("region") or ""
        if account_id and region and project_name:
            derived = f"storage-for-{project_name}-{account_id}-{region}"
            updated["s3_bucket"] = derived
            updated["s3_arn"] = f"arn:aws:s3:::{derived}"
            changed = True
    ...
    if changed:
        ...
        print(
            ...
            f"s3_bucket={updated.get('s3_bucket')}"
        )
...
def _project_secret_resource_arns(config) -> list:
    ...
    secret_arns = [
        ...
        # Shared secret named exactly "tavilyapikey" (AWS appends a 6-char suffix).
    ...
    return list(dict.fromkeys(secret_arns))
...
def _default_ecr_repository_name(project_name: str) -> str:
    current_folder_name = os.path.basename(script_dir)
    return f"{project_name}_{current_folder_name}"


def resolve_runtime_ecr_repository(
    ecr_client,
    config: dict,
    *,
    prefer_existing: bool = False,
) -> Optional[str]:
    """Return ECR repo for this project, falling back to config.ecr_repository.

    When projectName is corrected (e.g. strands-runtime → gsolution), the new
    ``{project}_strands`` repo may be empty. Reuse the previously recorded repo
    so AgentCore can be created/updated without a full rebuild.
    """
    project_name = config.get("projectName") or ""
    preferred = _default_ecr_repository_name(project_name) if project_name else ""
    fallback = (config.get("ecr_repository") or "").strip()
    candidates = []
    if prefer_existing and fallback:
    ...
    for repo in candidates:
        try:
            ...
            print(f"  ECR repository not found: {repo}")
            continue
        tag = resolve_ecr_image_tag(ecr_client, repo, config)
        if tag and verify_ecr_image_tag(ecr_client, repo, tag):
            if preferred and repo != preferred:
                print(
                    f"  ✓ Reusing existing ECR repository '{repo}' "
                    f"(project repo '{preferred}' has no usable image yet)"
                )
            return repo
        print(f"  No usable image tag in {repo}")
...
def push_to_ecr(*, skip_docker_build: bool = False, image_tag: Optional[str] = None):
    ...
    try:
        ...
        ecr_repository = _default_ecr_repository_name(project_name)
        print(f"ECR_REPOSITORY: {ecr_repository}")
        
        # Construct image tag and ECR URI
        build_image_tag = timestamp
        ecr_uri = f"{aws_account_id}.dkr.ecr.{aws_region}.amazonaws.com/{ecr_repository}:{build_image_tag}"
        ...
        if skip_docker_build:
            ...
            reuse_repo = resolve_runtime_ecr_repository(
                ecr_client, config, prefer_existing=True
            )
            if not reuse_repo:
                print(
            ...
            resolved_tag = resolve_ecr_image_tag(
                ecr_client, reuse_repo, config, image_tag=image_tag
            )
            if not resolved_tag:
                print(
                    "Error: No image tag found. Build and push an image from a host with Docker, "
        ...
        local_tag = f"{ecr_repository}:{build_image_tag}"
        ...
        update_config('latest_image_tag', build_image_tag)
...
def get_latest_image_tag(config):
    ...
    try:
        ...
        ecr_client = boto3.client('ecr', region_name=aws_region)
        repository_name = resolve_runtime_ecr_repository(
            ecr_client, config, prefer_existing=True
        )
        if not repository_name:
            print("Error: No ECR repository available for AgentCore runtime image")
            return None

        saved = resolve_ecr_image_tag(ecr_client, repository_name, config)
        if saved and verify_ecr_image_tag(ecr_client, repository_name, saved):
            update_config("ecr_repository", repository_name)
            print(f"Latest image tag: {saved}")
            return saved
