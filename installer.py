        ...
        try:
            ...
            if e.response["Error"]["Code"] == "ResourceNotFoundException":
                if key == "tavily":
                    ...
                    api_key = input(
                        f"Creating {secret_config['name']} - Tavily API Key: "
                    ).strip()
                ...
                try:
                    ...
                    logger.error(
                        f"  Failed to create secret {secret_config['name']}: {create_error}"
                    )
                    raise
            else:
...
def _ecs_agent_runtime_resource_arns() -> List[str]:
    ...
    Root installer project_name can differ from the Agent Runtime installer
    projectName. Include project_name, git_name, runtime_agent folder names,
    plus any explicit agent_runtime_arn from local configs.
    """
    ...
    configs = [app_config, *runtime_configs]
...
def build_and_push_docker_image(
    ...
    # Rebuild application/skills.list from skills before image build.
...
def main():
    ...
    try:
        ...
        app_environment = build_app_environment(
            ...
            knowledge_base_id,
            data_source_id,
            agentcore_memory_role_arn=agentcore_memory_role_arn or "",
        ...
        if secret_arns.get("tavily"):
            logger.info(f"  Tavily Secret: tavilyapikey-{project_name}")
        if s3_files_info:
            logger.info(f"  S3 Files session Access Point: {s3_files_info.get('access_point_arn')}")
            logger.info(
                f"  Agent Runtime Subnets: {', '.join(s3_files_info.get('subnets', []))}"
            )
        if s3_files_app_data_info:
            logger.info(
                f"  S3 Files app-data Access Point: {s3_files_app_data_info.get('access_point_arn')}"
            )
        if agentcore_memory_role_arn:
