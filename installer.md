...
- **S3 Files**: AgentCore 세션 checkpoint·ECS `tasks.db` 영속화를 위해 S3 버킷을 NFS로 마운트 (상세: [s3files.md](./s3files.md))
...
- **설정**:
  ...
  - 퍼블릭 액세스 차단
  - 버전 관리: S3 Files 연동 시 **Enabled** (`_ensure_s3_bucket_versioning_enabled`)
  - `docs/` 폴더 자동 생성
...
| `role-ecs-task-for-{project_name}-{region}` | ECS 태스크용 역할 (Bedrock, S3, AgentCore invoke, S3 Files mount 등) |
...
| S3 Files sync role | S3 Files ↔ S3 버킷 동기화용 역할 (`_get_or_create_s3files_sync_role`) |
...
│   ├── ALB SG (포트 80)
│   ├── ECS SG (포트 8501, 443)
│   ├── agent-runtime-sg-for-{project_name} (AgentCore Runtime)
│   └── S3 Files mount SG (NFS 2049)
└── VPC Endpoints
...
VPC 생성 직후 `[5.5/10]` 단계에서 프로비저닝합니다. 상세 동작은 [s3files.md](./s3files.md)를 참고하세요.

| 리소스 | 설명 |
|--------|------|
| File System | S3 버킷 `agentcore-sessions/` prefix 연동 |
| Access Point | AgentCore·ECS가 마운트하는 진입점 |
| Mount Targets | Private Subnet에 NFS(2049) 마운트 |
| Sync Role | S3 ↔ S3 Files 비동기 동기화 |
...
- Runtime이 VPC 모드로 배포되도록 `agent_runtime_vpc_subnets`·`agent_runtime_security_groups`를 `application/config.json`에 기록
...
| 리포지토리 | 용도 | Dockerfile |
|-----------|------|------------|
| `ecr-for-{project_name}` | Web UI (ECS) | 프로젝트 루트 `Dockerfile` (multi-stage: Node frontend + Python) |
...
- **이미지 태그**: 빌드 시 타임스탬프 태그 (`YYYYMMDDHHMMSS`) + ECR에서 `latest`로 promote
...
### 11. AgentCore LangGraph Runtime

`application/config.json`에 `sharing_url` 및 S3 Files 설정이 반영된 뒤, 루트 `installer.py`가 `runtime_agent/langgraph/installer.py`를 호출합니다 (`[11/10]`).
...
| AgentCore Runtime | `create_agent_runtime`으로 Runtime 생성/업데이트 (S3 Files + VPC) |
...
ECS Task Role에는 `bedrock-agentcore:*`, `secretsmanager:GetSecretValue`(Resource `*`) 등이 포함됩니다. S3 Files 마운트 권한은 이후 `ensure_ecs_task_s3files_policy()`로 추가됩니다.
...
def create_vpc() -> Dict[str, str]:
    ...
    # Bedrock Runtime + private subnet VPC endpoints
    #   (ECR, Logs, Secrets Manager, AgentCore, S3 gateway)
    return {
        "vpc_id": vpc_id,
...
#### `create_s3_files_session_storage()`
AgentCore·ECS용 S3 Files 세션 스토리지 프로비저닝
...
def create_s3_files_session_storage(
    ...
    *,
    ...
    # agent-runtime-sg + NFS SG 교차 규칙
    # ECS task role에 s3files mount 정책 부여
    # Agent Runtime SG를 VPC endpoint에 연결
    return {
...
def create_alb(vpc_info: Dict[str, str]) -> Dict[str, str]:
    ...
    # idle timeout 120초 설정 (ensure_alb_idle_timeout)
...
def build_and_push_docker_image(
    ...
    # 디스크 여유 확인 (_ensure_docker_disk_space)
    # docker buildx build --platform linux/arm64 --push
    # 타임스탬프 태그를 latest로 promote
...
def deploy_ecs_service(
    ...
    # ECS 클러스터 생성
    # IP 타겟 그룹 생성 (stickiness 활성화)
    # ALB 리스너 및 커스텀 헤더 규칙 생성
    # 태스크 정의 등록 (runtimePlatform=ARM64, APP_CONFIG_JSON,
    #   S3 Files 볼륨 /mnt/app-data 마운트)
    # Fargate 서비스 생성/업데이트 및 ready 대기
    return {
...
#### `apply_s3_files_config()`
S3 Files·Runtime VPC 설정을 `application/config.json`에 병합
...
컨테이너 런타임에 주입할 `application/config.json` 내용 생성. LangGraph Runtime의 `agent_runtime_arn`·`agent_runtime_role`을 `_merge_runtime_agent_settings()`로 병합합니다. S3 Files 필드는 `apply_s3_files_config()`로 추가됩니다.
...
| `create_vpc_endpoint()` | Interface VPC 엔드포인트 생성 |
| `create_s3_gateway_vpc_endpoint()` | S3 Gateway VPC 엔드포인트 생성 |
| `ensure_private_subnet_vpc_endpoints()` | ECR/Logs/Secrets/AgentCore/S3 엔드포인트 보장 |
...
| `generate_image_build_tag()` / `resolve_ecr_image_uri()` | 타임스탬프 태그 생성·기존 이미지 해석 |
...
| `create_s3_files_session_storage()` 관련 | `_get_or_create_s3files_*`, `ensure_ecs_task_s3files_policy`, `_ensure_agent_runtime_vpc_endpoint_access` 등 |
...
ARM64 EC2에서 Docker buildx로 `linux/arm64` 이미지를 빌드·push한 뒤 ECS Fargate(ARM64) 서비스를 생성합니다.  
...
[7/10] CloudFront 배포 생성
       ...
       ↓
[8/10] 설정·Agent Runtime·ECR·Docker
       • application/config.json 생성 (sharing_url + S3 Files 반영)
       • mcp.list / skills.list 동기화
       • [11/10] LangGraph Agent Runtime 설치
       • ECR 리포지토리 생성
       • Dockerfile 기반 linux/arm64 buildx 빌드 및 push
       ↓
[9/10] ECS Fargate 서비스 배포
       • CloudWatch Logs 그룹 생성
       • IP 타겟 그룹 + stickiness + ALB 리스너
       • 태스크 정의 등록 (S3 Files /mnt/app-data 마운트)
...
Summary:
  ...
  ECR Image: {account_id}.dkr.ecr.us-west-2.amazonaws.com/ecr-for-power-runtime:{build_tag}
...
| `agent_runtime_vpc_subnets` | AgentCore Runtime VPC 서브넷 목록 |
| `agent_runtime_security_groups` | AgentCore Runtime 보안 그룹 목록 |
| `latest_image_tag`, `build_number` | ECS 이미지 빌드 태그 |
...
ECS Web UI는 프로젝트 루트의 multi-stage `Dockerfile`로 빌드됩니다. Agent는 AgentCore runtime(`runtime_agent/langgraph/installer.py`)에서 별도로 `linux/arm64` 이미지로 배포됩니다.
...
- S3 Files Access Point ARN이 없으면 Runtime installer는 managed `sessionStorage` + `PUBLIC` 모드로 fallback합니다 ([s3files.md](./s3files.md))
...
| Docker 디스크 부족 | 정리 후 재시도, 부족 시 빌드 실패 |
